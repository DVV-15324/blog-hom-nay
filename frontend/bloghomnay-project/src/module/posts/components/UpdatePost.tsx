...
export default function UpdatePost() {
    ...
    function preserveSpaces(html: string): string {
        // Tách các đoạn code block và phần còn lại
        const codeBlockRegex = /<pre.*?>[\s\S]*?<\/pre>|<div class="ql-code-block-container"[\s\S]*?<\/div>/g;

        return html.replace(codeBlockRegex, (codeBlock) => {
            // Với code block: thay các chuỗi 2 dấu cách trở lên
            return codeBlock.replace(/ {2,}/g, (match) => '\u00A0'.repeat(match.length));
        }).replace(/^ +/gm, (match) => {
            // Với phần còn lại: thay dấu cách đầu dòng (1 hoặc nhiều)
