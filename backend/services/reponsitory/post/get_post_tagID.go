...
func (t *PostServiceSQL) GetsPostByTagsID(ctx context.Context, tagID []int) ([]entity.Posts, error) {
	...
	query := fmt.Sprintf(`SELECT p.id, p.user_id, p.category_id, p.title, p.slug, p.content,p.created_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&data.Id, &data.UserID, &data.CategoryId, &data.Title, &data.Slug, &data.Content, &data.CreatedAt, &data.UpdatedAt); err != nil {
			return nil, err
		}
